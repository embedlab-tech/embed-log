...
class VersionCommandTests(unittest.TestCase):
    ...
    def test_version_command_prints_source_and_commit(self):
        ...
        with patch(
            ...
            return_value=("1.1.5", "abc1234", "branch", "branch", "cli-version", ""),
        ):
            with redirect_stdout(buf):
                rc = _run_version(args)
        self.assertEqual(rc, 0)
        text = buf.getvalue()
        self.assertIn("[OK] version: 1.1.5", text)
    ...
    def test_main_version_flag_shows_source_and_commit(self):
        buf = io.StringIO()
        with patch("backend.cli.dispatch._display_version_line", return_value="embed-log 1.1.5 (branch:cli-version, abc1234)"):
            with redirect_stdout(buf):
                rc = main(["--version"])
        self.assertEqual(rc, 0)
        self.assertEqual(buf.getvalue().strip(), "embed-log 1.1.5 (branch:cli-version, abc1234)")
